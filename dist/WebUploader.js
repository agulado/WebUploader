...
(function () {
    ...
    function WebUploader() {
        ...
        return {
            // 获得进度条视图
            getProgressView: function getProgressView(opt) {
                ...
                if (this.opt.show_kind == 1) {
                    ...
                    // autostart
                    if (this.opt.autoStart === true) this.dom_obj.button_start.trigger("click");
                } else return this.dom_obj.wrapper;
            },
            // 关闭进度条视图
            ProgressViewClose: function ProgressViewClose() {
                debug("\n87 ProgressViewClose()");

                this.Upload_abort();

                if (this.dom_obj) {
                    this.dom_obj.wrapper_bg && this.dom_obj.wrapper_bg.css("display", "none");
                    this.dom_obj.wrapper && this.dom_obj.wrapper.css("display", "none");
            ...
            create_dom: function create_dom() {
                ...
                // 背景层
                if (this.opt.show_kind == 1) this.dom_obj.wrapper_bg = $(document.createElement("div")).css({
                    "display": "none",
                    "position": "fixed",
                    "width": "100vw",
                    "height": "100vh",
                ...
                // 大盒儿
                this.dom_obj.wrapper = $(document.createElement("div")).css({
                    "position": "relative",
                ...
                if (this.opt.show_kind == 1) this.dom_obj.wrapper.css({
                    "display": "none",
                    "position": "fixed",
                ...
                // 获得大盒宽
                var wrapper_width = this.dom_obj.wrapper.width();

                // 表头盒儿
                this.dom_obj.title_ul = $(document.createElement("ul")).css({
                    "width": this.dom_obj.wrapper.width() + "px",
                    "height": "" + opt_global.li_height + opt_global.li_height_unit
                }).appendTo(this.dom_obj.wrapper);

                // 文件盒儿
                this.dom_obj.file_ul = $(document.createElement("ul")).css({
                    "width": this.dom_obj.wrapper.width() + "px",
                ...
                // li模版
                this.dom_obj.file_li_template = $(document.createElement("li")).css({
                    "display": "table",
                    "width": this.dom_obj.wrapper.width() + "px",
                    "height": "" + opt_global.li_height + opt_global.li_height_unit,
                ...
                // cell模板
                this.dom_obj.file_span_template = $(document.createElement("span")).css({
                    "display": "table-cell",
                ...
                this.dom_obj.file_span_template.clone().addClass("cell1").css({
                    "width": wrapper_width * 0.38 + "px",
                    "padding": "0 " + wrapper_width * 0.01 + "px"
                }).appendTo(this.dom_obj.file_li_template);

                // cell2 - 大小
                this.dom_obj.file_span_template.clone().addClass("cell2").css({
                ...
                // cell4 - 操作/状态
                this.dom_obj.file_span_template.clone().addClass("cell4").css({
                    "width": "10%",
                    "text-align": "center"
                }).html(this.dom_obj.file_span_cell4_button_delete_html).appendTo(this.dom_obj.file_li_template).find("span").css({
                    "cursor": "pointer"
                });
                ...
                var title_span = this.dom_obj.file_li_title.find("span");
                title_span.css({
                    "text-align": "center",
                    "font-weight": "bold"
                });
                ...
                // 按钮组
                this.dom_obj.buttons = $(document.createElement("ul")).css({
                    "position": "absolute",
                    "width": "100%",
                ...
                // 按钮 - 关闭
                this.dom_obj.button_close = $(document.createElement("li")).css({
                    "float": "left",
                    ...
                    "color": this.opt.wrapper_font_color,
                    "cursor": "pointer"
                }).text("取消并关闭").appendTo(this.dom_obj.buttons);

                // 按钮 - 开始上传
                this.dom_obj.button_start = $(document.createElement("li")).css({
                    "float": "right",
                    "box-sizing": "border-box",
                    "width": "50%",
                    "height": "" + this.opt.button_height + this.opt.button_height_unit,
                    ...
                    "color": this.opt.wrapper_font_color,
                    "cursor": "pointer"
                }).text("开始上传").appendTo(this.dom_obj.buttons);
            },
            // 按钮监听
            buttonsLisenter: function buttonsLisenter() {
                var _this2 = this;
                ...
                // 开始上传
                this.dom_obj.button_start.unbind("click").on("click", function (e) {
                    ...
                    setTimeout(function () {
                        ...
                        _this.addClass("disable").css({
                            "color": _this2.opt.wrapper_font_color,
                            "text-decoration": "none",
                            "cursor": "default"
                        }).text("上传中，请稍候");

                        var cell4_button = _this2.dom_obj.file_ul.find(".cell4");
                        cell4_button.html(_this2.dom_obj.file_span_cell4_button_wait_html);
                        $(cell4_button[0]).html("0%");

                        debug("\n357 button_start.click. files=");
                        debug(_this2.opt.files);
                        debug("\n366 button_start.click. opt=");
                        debug(_this2.opt);

                        _this2.UploadStart({
                            files: _this2.opt.files,
                            upload_url: _this2.opt.upload_url,
                            check_url: _this2.opt.check_url,
                            thread_maxCount: _this2.opt.thread_maxCount,
                            callback_successAll: _this2.opt.callback_successAll
                        });
                    }, 0);
                });

                // hover效果
                this.dom_obj.buttons.find("li").hover(function (e) {
                    var _this = $(e.target);
                    if (_this.hasClass("disable")) return;

                    _this.css({
                        "color": _this2.opt.wrapper_border_color,
                ...
                // 关闭
                this.dom_obj.button_close.unbind("click").on("click", function () {
                    _this2.ProgressViewClose();
                });

                // 删除
                this.dom_obj.file_ul.find(".cell4 span").hover(function (e) {
                    $(e.target).parents(".cell4").find("svg").css({
                        "fill": _this2.opt.wrapper_border_color
                    });
                }, function (e) {
                    $(e.target).parents(".cell4").find("svg").css({
                        "fill": _this2.opt.wrapper_font_color
                    });
                }).unbind("click").on("click", function (e) {
                    var _this = $(e.target),
                        li = _this.parents("li");

                    if (_this.css("cursor") != "pointer") return;

                    debug("\n354 li.remove()\u524D: this.opt.files=");
                    debug(_this2.opt.files);

                    _this2.opt.files.splice(li.index(), 1);
                    li.remove();

                    debug("\n360 li.remove()\u540E: this.opt.files=");
                    debug(_this2.opt.files);

                    if (_this2.opt.files.length === 0) _this2.ProgressViewClose();
                });
            },
            ...
            Uploading_index: 0,
            // 开始上传
            UploadStart: function UploadStart(opt) {
                var _this3 = this;
                ...
                var i = 0,
                    ...
                    para = {
                    hasError: [],
                    upload_url: this.opt_upload.upload_url,
                    check_url: this.opt_upload.check_url,
                    updateID: this.updateID,
                    callback_progress: function callback_progress(event_progress, loadedSize, totalSize, index) {
                    ...
                    UploadSuccessCounter_success: function UploadSuccessCounter_success(filePath) {
                        ...
                        _this3.UploadSuccessFilepath = {};
                        _this3.UploadSuccessCount = 0;

                        if (para.hasError.length > 0) {
                            if (_this3.dom_obj) {

                                _this3.opt.callback_successAll_hasError && _this3.opt.callback_successAll_hasError(filePath);

                                var reStart = function reStart() {
                                    var li = _this3.dom_obj.file_ul.find("li");
                                    ...
                                    i = 0, len = _this3.opt.files.length;
                                    for (var arr = []; i < len; i++) {
                                        if (_this3.opt.files[i] === "") continue;
                                        arr.push(_this3.opt.files[i]);
                                    }
                                    _this3.opt.files = arr;

                                    para.hasError = [];

                                    debug("\n483: \u91CD\u8BD5\u540E\u7684files=");
                                    debug(_this3.opt.files);
                                };

                                _this3.dom_obj && _this3.dom_obj.button_start.text("重试").css({
                                    "cursor": "pointer"
                                }).removeClass("disable").one("click", reStart);
                            } else {
                                _this3.opt_upload.callback_successAll_hasError && _this3.opt_upload.callback_successAll_hasError(para.hasError, filePath);
                            }
                        } else {
                            _this3.opt_upload.callback_successAll && _this3.opt_upload.callback_successAll(filePath);
                        }
                    },
                    UploadSuccessCounter_next: function UploadSuccessCounter_next() {
                        var i = ++_this3.Uploading_index;
                        if (i < _this3.opt_upload.files.length) {
                            para.file = _this3.opt_upload.files[i];
                            para.index = i;
                            para.index_flag = _this3.opt_upload.filesIndexFlag[i];
                            _this3.Upload_do(para);
                        }
                    },
                    callback_success: function callback_success(index, filePath) {

                        if (_this3.opt_upload.updateID) debug("\n112: UploadStart callback_success. index=" + index + ",filePath=" + filePath);

                        if (_this3.opt_upload.callback_success) _this3.opt_upload.callback_success(index, filePath);

                        setTimeout(function () {
                            _this3.UploadSuccessCounter(_this3.opt_upload.files.length, para.UploadSuccessCounter_success, para.UploadSuccessCounter_next);
                        }, 0);
                    },
                    callback_error: function callback_error(index) {

                        para.hasError.push(index);

                        if (_this3.opt_upload.callback_error) _this3.opt_upload.callback_error(index);
                        if (_this3.dom_obj) {
                            var li = _this3.dom_obj.file_ul.find("li:eq(" + index + ")"),
                                percent_span = li.find(".cell4");
            ...
            UploadSuccessCounter: function UploadSuccessCounter(count, _success_callback, _next_callback) {
                if (++this.UploadSuccessCount >= count) _success_callback && _success_callback(this.UploadSuccessFilepath);else _next_callback && _next_callback();
            },

            // 上传文件
            Upload_do: function Upload_do(opt) {
                var _this4 = this;
                ...
                if (opt.file === "") opt.callback_success && opt.callback_success();
                    ...
                    var ajax_check = function ajax_check() {
                        $.ajax({
                            ...
                            error: function error(err) {
                                ...
                                if (err.statusText != 'abort') opt.callback_error && opt.callback_error(opt.index);
                            }
                        ...
                        _this4.Xhr = $.ajax({
                            ...
                            xhr: function xhr() {
                                ...
                                Xhr.upload.addEventListener("progress", function (e) {
                                    opt.callback_progress && opt.callback_progress(e, split_start + e.loaded, opt.file.size, opt.index);
                                });
                                return Xhr;
                            },
                            success: function success(res) {

                                debug("\n636: upload success. this.dom_obj=");
            ...
            // 取消上传
            Upload_abort: function Upload_abort() {
    ...
    // 获得进度条视图的opt_assign
    var getProgress_opt_assign = function getProgress_opt_assign(opt) {
        ...
        // 设置debug
        if (opt.debug !== undefined) opt_global.debug = opt.debug;

        return Object.assign(opt_default, opt);
    };

    // 开始上传的opt_assign
    var UploadStart_opt_assign = function UploadStart_opt_assign(opt) {

        var opt_default = {
            files: [], // 上传文件列表，Filelist或array都可以
            filesIndexFlag: [], // 上传文件对应的index标识，在返回的对象中作为key
            upload_url: null, // ajax上传地址
            check_url: null, // ajax检查已传部分地址
    ...
    // 格式化size
    var sizeFormat = function sizeFormat(size_total) {
        var size_perUnit = 1024;
        var size_percent = size_total / size_perUnit;
        var size_level = 0; // 0-KB 1-MB 2-G
        var size_unit = ["KB", "MB", "G"];
    ...
    // debug
    var debug = function debug(message, act) {
        if (opt_global.debug !== true) return;
    ...
    var FileListToFileArray = function FileListToFileArray(filelist) {

        // FileList会有item方法
        if (!filelist.item) return filelist;
